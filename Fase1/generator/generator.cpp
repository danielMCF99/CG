...
void trianglesToFile(vector<Triangle> t, string f) {
	...
	unsigned int i = 0;
	while (i < t.size()) {
		file << "Triangle " << i << ":" << " " << ((rand() % 3) * 0.25) << " " << ((rand() % 3) * 0.5) << " " << ((rand() % 3) * 0.4) << " , " << get<0>(t[i].triangle).toString() << " , " << get<1>(t[i].triangle).toString() << " , " << get<2>(t[i].triangle).toString() << endl;
		i++;
	}
}
...
void drawCone(float radius, float height, int slices, int stacks, string name) {
	...
	for (int i = 0; i <= slices; i++) {
		...
		// BASE DO CONE
		Vertex p1(sin(delta_alpha + alpha) * radius, 0.0f, cos(delta_alpha + alpha) * radius);
		Vertex p2(0.0f, 0.0f, 0.0f);
...
void createXML() {
	...
	doc.SaveFile("../engine/cenario.xml");
}

void updateXML(const char* file) {
	XMLDocument doc;
	if (doc.LoadFile("../engine/cenario.xml") == XML_SUCCESS) {
